...
public class ClientsWindowController implements Initializable, NotifyableClientEntityTreeChange {
    ...
    private void onMouseDoubleClick() {
        ...
        new ClientPropertyWindowView(NetworkService.getInstance().getEntityTree().getNetworkClientEntity(clientID),clientsWindowView);
    }
    ...
    private synchronized void init() {
        NetworkClientEntity rootEntity = NetworkService.getInstance().getEntityTree().getRoot();

        TreeItem rootChild = createTreeItem(rootEntity.getNickname(),
                UserID.getInstance().getUserID().equals(rootEntity.getID()),
                UserID.getInstance().getUserID().equals(rootEntity.getID()),
                rootEntity.getID().toString());


        rootItem.getChildren().add(rootChild);
        loadChildData(rootChild, NetworkService.getInstance().getEntityTree(),NetworkService.getInstance().getEntityTree().getRoot());

        setMouseDoubleClickEvent();
        NetworkService.getInstance().subscribeForClientEntityTreeChange(this);
    }
    ...
    private void loadChildData(TreeItem viewRoot, NetworkClientEntityTree entityTree, NetworkClientEntity entityTreeRoot) {
        ...
        for(NetworkClientEntity act : children) {
            TreeItem<Text> child = createTreeItem(act.getNickname(),UserID.getInstance().getUserID().equals(act.getID()),
                    UserID.getInstance().getUserID().equals(act.getID()),act.getID().toString());
    ...
    public void onClose() {
        NetworkService.getInstance().unSubscribeForClientEntityTreeChange(this);
    }
