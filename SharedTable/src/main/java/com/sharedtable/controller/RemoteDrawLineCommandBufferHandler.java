...
/**
 * Ha nem választjuk szét userenként és lezárulási idő szerint a commandokat, akkor összemosódnak egy state-be
...
public class RemoteDrawLineCommandBufferHandler {
    ...
    public void closeMemento(UUID userID, UUID mementoID,boolean isLinked, UUID after,UUID before) {
        canvasController.insertRemoteMementoAfter(mementoID,commandBuffers.get(userID),isLinked,userID,after,before);
        //printAllCommands(commandBuffers.get(userID));
        commandBuffers.remove(userID);
        if(!isLinked)
            canvasController.processStateChangeCommand(mementoID); //a láncolási szakadásokat figyelembe véve újra rajzol
    }
