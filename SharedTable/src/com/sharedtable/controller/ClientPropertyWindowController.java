...
public class ClientPropertyWindowController implements Initializable {
    ...
    class PingThread extends Thread {
        ...
        public void run(){
            disablePingButton();
            long sum = 0;
            long act = NetworkService.pingClient(entity.getID());
            updatePingLabel(act);
            sum += act;
            act = NetworkService.pingClient(entity.getID());
            updatePingLabel(act);
            sum += act;
            act = NetworkService.pingClient(entity.getID());
            updatePingLabel(act);
        ...
        private void restorePingButton(){
            Platform.runLater(() -> {
    ...
    public void setData(NetworkClientEntity entity) {
        ...
        if(entity.getID().equals(UserID.getUserID())) {
            pingButton.setDisable(true);
